...
106. 2026-05-20 17:54:47: Prepped `Model-Viewport-5 / Phase 6.3 - Clay Studio Lighting And AO Tuning` inside the dedicated future doc after screenshot review showed the shipped AO is stable but the visual target still needs softer Clay Studio lighting, reduced directional/key-light shadow harshness, broader lower-contrast AO presets, and possible Clay Studio edge de-emphasis before any advanced UI sliders.
105. 2026-05-20 17:43:27: Fixed the `Model-Viewport-5 / Phase 6.2` Ambient Occlusion black-screen regression after user review by adding an `OutputPass` after `SSAOPass` in the post-processing runtime, preserving the shipped Properties `Ambient Occlusion` presets while keeping the composer output path visible.
104. 2026-05-20 17:35:27: Marked `Model-Viewport-5 / Phase 6.2 - Basic Clay Studio And Ambient Occlusion Controls` shipped inside the dedicated future doc after Properties `Render` gained an `Ambient Occlusion` `Off` / `Low` / `Medium` / `High` select backed by shared `ViewSettings.postProcessing` preset/read helpers, preserving `Viewport Style`, render-preview settings, persistence policy, and focused Properties plus store/helper proof.
103. 2026-05-20 17:31:13: Prepped `Model-Viewport-5 / Phase 6.2 - Basic Clay Studio And Ambient Occlusion Controls` inside the dedicated future doc after reading the shipped Properties `Viewport Style` group, normalized `ViewSettings.postProcessing` contract, SSAO runtime mappings, and Properties test seam, locking the next code slice to a compact `Ambient Occlusion` `Off` / `Low` / `Medium` / `High` select, a small shared preset/read helper, `setViewKey('postProcessing', ...)` writes, and focused Properties plus store/helper proof without advanced numeric sliders.
102. 2026-05-20 17:29:02: Marked `Model-Viewport-5 / Phase 6.1 - Properties Render Post-Process Placement` shipped inside the dedicated future doc after Properties `Render` gained a compact `Viewport presentation` group, a `Viewport Style` `Standard` / `Clay Studio` select wired through `ViewSettings.viewportStyle`, existing render-preview quality control preservation, and focused Properties proof while keeping Ambient Occlusion presets deferred to Phase 6.2.
101. 2026-05-20 17:23:21: Prepped `Model-Viewport-5 / Phase 6.1 - Properties Render Post-Process Placement` inside the dedicated future doc after reading the live `PropertiesRenderSection.tsx` and `PropertiesSurface.test.tsx` seams, locking the next code slice to one compact interactive viewport presentation group in Properties `Render`, a `Viewport Style` `Standard` / `Clay Studio` select wired through `ViewSettings.viewportStyle`, existing render-preview control preservation, and no Ambient Occlusion presets until Phase 6.2.
100. 2026-05-20 17:21:33: Marked `Model-Viewport-5 / Phase 6 - Properties Render Controls Umbrella` shipped inside the dedicated future doc as a doc/ownership closeout, confirming Properties `Render` as the Clay Studio/SSAO tuning and readback home, preserving `Shift+D` as the quick style switch, locking `Viewport Style` and `Ambient Occlusion` as the simple labels, and leaving actual control implementation to Phase 6.1 and Phase 6.2.
99. 2026-05-20 17:18:52: Prepped `Model-Viewport-5 / Phase 6 - Properties Render Controls Umbrella` inside the dedicated future doc after reading the live Properties `Render` section and shipped Clay Studio/SSAO runtime path, locking Properties `Render` as the tuning/readback home, preserving `Shift+D` as the quick style switch, reserving `Viewport Style` and `Ambient Occlusion` as the simple labels, and keeping actual control implementation split into Phase 6.1, Phase 6.2, and Phase 6.3.
98. 2026-05-20 17:15:18: Marked `Model-Viewport-5 / Phase 5 - Overlay And Selection Compatibility` shipped inside the dedicated future doc after focused viewer proof covered SSAO-safe topology selected/hover overlays, display-edge `off` / `on` / `visibleOnly` behavior, sketch/extrude overlay material contracts, render-preview separation, and axis HUD rendering outside the composer path without requiring private SSAO pass surgery or broad highlight retuning.
97. 2026-05-20 17:02:33: Prepped `Model-Viewport-5 / Phase 5 - Overlay And Selection Compatibility` inside the dedicated future doc after reading shipped SSAO runtime and live overlay seams, locking the next implementation to proof/protection for topology hover/selection overlays, display-edge overlays, sketch/extrude overlays, transform helpers, axis HUD, and DOM HUD/radial overlays without broad highlight retuning or new controls.
96. 2026-05-20 16:47:11: Marked `Model-Viewport-5 / Phase 4 - Interactive SSAO Pass` shipped inside the dedicated future doc after `postProcessingRuntime` gained Three.js `SSAOPass` ownership, normalized SSAO quality/radius/intensity mapping, live settings updates, kernel-change runtime recreation, direct fallback preservation, render-preview separation, focused viewer/runtime proof, and production build verification.
95. 2026-05-20 16:41:17: Prepped `Model-Viewport-5 / Phase 4 - Interactive SSAO Pass` inside the dedicated future doc after reading the shipped Phase 3 composer helper and the installed Three.js `SSAOPass` API, locking the next implementation to adding `SSAOPass` inside `postProcessingRuntime`, mapping normalized `ViewSettings.postProcessing` quality/radius/intensity settings, preserving direct/render-preview fallback behavior, and proving settings updates without geometry rebuilds.
94. 2026-05-20 16:38:33: Marked `Model-Viewport-5 / Phase 3 - Post-Process Composer Boundary` shipped inside the dedicated future doc after the viewer gained a small `postProcessingRuntime` helper, opt-in post-process render branch, direct-render fallback for setup/render failures, resize/dispose lifecycle handling, and render-preview separation with focused viewer proof and production build verification.
93. 2026-05-20 16:23:50: Prepped `Model-Viewport-5 / Phase 3 - Post-Process Composer Boundary` inside the dedicated future doc after re-reading the live viewer render loop, resize, dispose, and render-preview seams, locking the next implementation slice to a small `postProcessingRuntime` helper, direct-render fallback, resize/dispose ownership, and focused viewer proof without SSAO, UI controls, SSGI, or render-preview/path-tracer changes.
92. 2026-05-20 16:15:53: Added the Properties workspace `Render` control ladder inside the dedicated `Model-Viewport-5` future doc, splitting the prior broad controls pass into `Phase 6 - Properties Render Controls Umbrella`, `Phase 6.1 - Properties Render Post-Process Placement`, `Phase 6.2 - Basic Clay Studio And Ambient Occlusion Controls`, and `Phase 6.3 - Advanced Post-Process Tuning And Status` while keeping `Shift+D` as the fast Clay Studio switch.
91. 2026-05-20 16:05:05: Marked `Model-Viewport-5 / Phase 2.1 - Shift+D Clay Studio Wheel Entry` shipped inside the dedicated future doc after the existing display-mode menu hook and `ViewerHost` radial wheel gained a Clay Studio style action wired through `ViewSettings.viewportStyle`, with normal display-mode selections resetting the style to `standard`, existing edge-display controls preserved, and focused hook plus `ViewerHost` proof.
90. 2026-05-20 16:01:13: Added `Model-Viewport-5 / Phase 2.1 - Shift+D Clay Studio Wheel Entry` inside the dedicated future doc after user review caught that Phase 2 shipped Clay Studio runtime support without a `Shift+D` display-wheel entry, reserving the narrow follow-up for routing the wheel to `ViewSettings.viewportStyle` while keeping SSAO controls and composer work deferred.
89. 2026-05-20 15:50:43: Marked `Model-Viewport-5 / Phase 2 - Clay Studio Viewer Presentation Preset` shipped inside the dedicated future doc after the normalized `ViewSettings.viewportStyle` switch and rendered-mode-only Clay Studio viewer overrides landed with pale clay material, bright background/grade, pale ground, soft lights, runtime grid/axis suppression, render-preview exclusion, and focused store/viewer proof.
88. 2026-05-20 15:41:12: Prepped `Model-Viewport-5 / Phase 2 - Clay Studio Viewer Presentation Preset` inside the dedicated future doc after re-reading the live viewer display-mode, material-cache, environment, ground, grid/axes, and display-mode proof seams, locking the next implementation to a minimal `ViewSettings.viewportStyle` switch plus rendered-mode-only Clay Studio presentation overrides without SSAO, composer, UI controls, material-truth mutation, or render-preview behavior changes.
87. 2026-05-20 15:36:45: Marked `Model-Viewport-5 / Phase 1 - Clay Studio And Post-Process Settings Contract` shipped inside the dedicated future doc after `ViewSettings.postProcessing` gained the SSAO settings contract, defaults, normalization, view-persistence policy handling, viewer clone preservation, and focused store/persistence proof without changing viewer runtime rendering yet.
...
- `Model-Viewport-5 / Phase 1 - Clay Studio And Post-Process Settings Contract` shipped the additive `ViewSettings.postProcessing` owner for SSAO enablement, intensity, radius, and quality without changing viewer runtime rendering
- `Model-Viewport-5 / Phase 2 - Clay Studio Viewer Presentation Preset` shipped the minimal `ViewSettings.viewportStyle` owner and rendered-mode-only Clay Studio viewer overrides
- `Model-Viewport-5 / Phase 2.1 - Shift+D Clay Studio Wheel Entry` shipped the narrow UI-routing follow-up that makes the shipped Clay Studio style selectable from the existing display wheel
- `Model-Viewport-5 / Phase 3 - Post-Process Composer Boundary` shipped the small composer helper, opt-in render branch, direct-render fallback, resize/dispose lifecycle, and render-preview separation
- `Model-Viewport-5 / Phase 4 - Interactive SSAO Pass` shipped Three.js `SSAOPass` through the composer helper with normalized SSAO setting mapping, live updates, quality-change recreation, direct fallback, and render-preview separation
- `Model-Viewport-5 / Phase 5 - Overlay And Selection Compatibility` shipped focused proof that SSAO preserves topology selection/hover overlays, display-edge modes, sketch/extrude overlays, render-preview separation, and axis HUD rendering without needing private pass surgery
- `Model-Viewport-5 / Phase 6 - Properties Render Controls Umbrella` shipped the ownership pass that keeps Properties `Render` as the Clay Studio/SSAO tuning home and keeps `Shift+D` as the quick style switch
- `Model-Viewport-5 / Phase 6.1 - Properties Render Post-Process Placement` shipped the first code-changing Properties slice with a compact `Viewport Style` control while preserving render-preview controls
- `Model-Viewport-5 / Phase 6.2 - Basic Clay Studio And Ambient Occlusion Controls` is now shipped as the Properties slice for `Ambient Occlusion` `Off` / `Low` / `Medium` / `High` presets backed by shared post-processing mappings
- `Model-Viewport-5 / Phase 6.3 - Clay Studio Lighting And AO Tuning` is now shipped as the visual tuning slice that reduces harsh Clay Studio directional/key-light shadowing, increases soft fill/ambient contribution, broadens AO preset contact depth, and de-emphasizes heavy Clay Studio display-edge overlays without adding advanced controls yet
- `Model-Viewport-5 / Phase 6.4 - Clay Studio Ground Contact` is now shipped as the follow-up slice that adds Clay Studio-only presentation contact shadows under visible base meshes so the viewer ground plane reads more anchored without changing non-Clay shadows, render preview, graph geometry, material truth, or export truth
- the plan splits the work into a settings contract, Clay Studio presentation preset, post-process composer boundary, SSAO runtime, overlay compatibility, Properties `Render` controls, performance/fallback polish, visual QA, and SSGI feasibility
