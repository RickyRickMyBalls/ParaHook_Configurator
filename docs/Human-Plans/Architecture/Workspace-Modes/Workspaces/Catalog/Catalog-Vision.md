...
### Doc History
66. 2026-04-21 19:58:57: Marked `Catalog-Gen2-HLG-23` complete after `Catalog-Gen2-18` closed the live PubParts metadata wishlist with runtime `parts.json` refresh, baked-cache fallback, ParaHook-owned normalized projection, stable source-key PubParts item IDs, and source-update preservation boundaries while keeping production hosting CORS/proxy deployment and live resources widening as future-owner work.
65. 2026-04-21 19:37:28: Added `Catalog-Gen2-HLG-23` after the live PubParts `.json` endpoint discussion and the user's wishlist that Zinc-added PubParts entries should appear in ParaHook after app refresh, clarifying that PubParts structured metadata should become the live upstream source while ParaHook still owns normalized filters, source options, source-library state, and Import/project handoff.
64. 2026-04-21 17:45:20: Marked `Catalog-Gen2-HLG-22` complete after `Catalog-Gen2-17 / Phase 5` audited the Phase 1-4 stack with focused materialization, trusted-provider, source-library metadata index, Catalog source-options, and production build verification, confirming PubParts Add To Project can truthfully attempt legal direct/browser/provider ZIP byte materialization, reuse OPFS/Internal Library and the existing ZIP list/preview/select/stage path, keep Upload ZIP fallback, keep the metadata index blob-free, and leave Import review as project-acceptance owner while persistent metadata storage, deployed provider infrastructure, richer UI surfacing, live end-to-end provider proof, STEP, builder, compatibility, eager scan, and bulk archive import remain future-owner work.
63. 2026-04-21 16:14:22: Added `Catalog-Gen2-HLG-22` after the PubParts/Dropbox buffer-streaming discussion, clarifying that Generation 2 should attempt direct in-memory ZIP byte materialization from browser-readable or trusted provider sources before falling back to user-uploaded ZIPs, while keeping OPFS as binary storage and routing SQLite/IndexedDB to metadata indexing instead of localStorage binary ownership.
62. 2026-04-21 15:49:20: Marked `Catalog-Gen2-HLG-19` complete after the OPFS/Internal Library and optional Local Library mirror lane shipped source-options OPFS caching/reopen, Home Page folder connect/reconnect/disable status, Catalog mirror status, best-effort visible-folder writes from explicit app-owned archive/extracted blobs, OPFS-unavailable mirror proof, and no Dropbox bypass or Import/project auto-commit.
61. 2026-04-21 15:21:03: Marked `Catalog-Gen2-HLG-21` complete after `Catalog-Gen2-16` shipped the source-options-first uploaded/Internal Library cached PubParts ZIP entry `3D` preview path with one-entry extraction, existing Catalog preview viewport reuse, object URL cleanup proof, disabled metadata-only/no-byte/unsupported rows, no silent Dropbox fetch, and no Import/project/builder/compatibility side effects.
60. 2026-04-21 14:52:29: Added `Catalog-Gen2-HLG-21` and routed uploaded or cached PubParts ZIP entry 3D preview into a new `Catalog-Gen2-16 - Uploaded ZIP Entry 3D Preview` lane, keeping source-options preview first, OPFS/dialog archive bytes as the only preview byte sources, existing Catalog preview viewport reuse as the preferred renderer, object URL cleanup as a required proof, and Import/project acceptance boundaries intact.
...
- [x] `Catalog-Gen2-HLG-22. make PubParts Add To Project try direct in-memory source-byte materialization when a source URL, trusted PubParts proxy, helper, or native bridge can legally provide ZIP bytes, then store successful archives in the Internal Library and fall back to Upload ZIP when browser access is blocked` - complete for the direct materialization lane with truthful source-byte status/fallback reads, Add To Project-triggered browser ZIP attempt and Internal Library cache reuse, trusted-provider boundary without browser secrets or mandatory infrastructure, OPFS/Internal Library large-byte ownership, Upload ZIP fallback, metadata-only source-library index, and unchanged Import review project-acceptance ownership.
- [x] `Catalog-Gen2-HLG-23. make PubParts structured metadata the live upstream source of truth so new PubParts parts added by Zinc can appear in ParaHook after app refresh without requiring a developer to regenerate ParaHook's baked cache, while ParaHook still owns normalized filters, source options, local library state, and Import/project handoff` - complete for the `Catalog-Gen2-18` lane with live `parts.json` metadata refresh, baked-cache fallback, ParaHook-owned taxonomy projection, stable source-key PubParts item IDs, and no automatic ZIP/import/project deletion or byte materialization.
...
After the browser-honest ZIP upload path, Gen2 should also try a smoother source-byte materialization lane. If a PubParts/Dropbox source URL or a trusted PubParts proxy, helper, or native bridge can deliver ZIP bytes after explicit user action, ParaHook should read those bytes into memory, inspect the ZIP, write the archive and extracted candidates into OPFS/Internal Library, optionally mirror app-owned files to Local Library, and reuse the existing preview/staged Import review flow. This does not mean localStorage binary storage, silent Dropbox bypass, or guaranteed browser access; blocked CORS/API reads still fall back to `Open Source` plus `Upload ZIP`.

PubParts structured metadata should be live upstream truth. The baked generated cache is still useful as fallback, fixture, and offline baseline, but the normal app/Catalog refresh path should ask `https://pubparts.xyz/parts.json` for the latest part records and run successful records through ParaHook's existing PubParts normalizer and Catalog projection. A live metadata refresh may add cards and update source metadata, preview images, platform labels, type labels, source pages, and archive links. It must not automatically download linked ZIP/model bytes or delete local source-library/import state when upstream metadata changes.

Large binary source bytes should live in OPFS/Internal Library and optional visible mirror files, not in `localStorage`. A later SQLite/IndexedDB metadata index may improve searching, versioning, source freshness, archive manifests, preview/importable status, and local-library bookkeeping, but it should index file ownership instead of becoming a hidden model-blob owner by default.

Uploaded or cached ZIP entry preview should be a separate Gen2 lane from Internal Library storage. Once a source-options dialog has user-granted archive bytes, either from the current uploaded ZIP or a same-source-version OPFS cache hit, Catalog may let the user preview one supported entry as `3D` before staging anything into Import review. The default browse and source-options read should still favor image/source metadata; the `3D` entry preview should be an explicit row-level action or toggle. The preferred renderer is the existing `CatalogCardPreviewViewport` and reference asset loader path, adapted for a source-options surface if needed, not a second Three.js viewer. Preview object URLs must be revoked when the preview changes, the dialog closes, or the surface unmounts. Metadata-only localStorage archive manifests, stale OPFS cache records, unsupported entries, and blocked Dropbox bytes without a user-granted archive must not silently become preview sources.
...
- direct in-memory source-byte materialization for PubParts ZIP/source links when browser fetch, trusted PubParts proxy, helper, or native bridge can legally provide bytes after explicit user action
...
- source-library metadata indexing for item/source version, archive manifest, preview/importable status, Internal Library state, and optional Local Library mirror status while OPFS or real files remain the large-binary owners
- runtime PubParts structured metadata refresh from live upstream `.json` with baked-cache fallback and ParaHook-owned normalization
- uploaded or cached PubParts ZIP entry `3D` preview inside source options when a current dialog archive blob or same-source-version Internal Library archive cache provides the bytes
...
- bypassing CORS, Dropbox API rules, or browser fetch restrictions just because a source-byte lane streams bytes into memory
...
- using `localStorage` for ZIP/model binaries or making SQLite the unreviewed primary blob owner for large archives/models
...
- silently fetching blocked Dropbox ZIP bytes only to preview, previewing metadata-only archive manifests as if bytes exist, or treating source-options `3D` preview as Import review staging/project acceptance
...
Create `Generation 2` family phases when work is about external source records, source adapters, PubParts mapping, live PubParts structured metadata refresh, full cached PubParts population, eager external preview image display, linked model entries, linked archive handoff, visible PubParts download staging, downloaded-versus-imported status, staged download clear controls, user-triggered external source inspection/import handoff, Dropbox shared-link direct-file resolution, shared ZIP/archive candidate inspection, efficient one-archive-at-a-time ZIP listing and selected extraction, staged importer/source-options UX, browser-honest ZIP download/upload handoff, direct in-memory source-byte materialization when a source/provider can legally provide bytes, trusted PubParts proxy/helper/native byte-provider boundaries, source-library metadata indexing, preview affordances for previewable staged entries, source-options `3D` preview of one uploaded/cached ZIP entry before Import staging, OPFS-backed Internal Library caching, optional Local Library folder mirroring, local PubParts library folder status, per-item local source manifests, source attribution, repo-backed XR/ADV asset-folder intake, versioned multi-file `3D` asset sets, or pre-built PubWheel entries.
