...
### `Import-3 Phase 12`
- [ ] `45. Post-Accept Imported Reference Load Failures Are Reproduced And Explained`
- [ ] `46. Accepted Import Asset Ownership Survives Draft Teardown`
- [ ] `47. The Browser Error State Exposes Honest Recovery Information`
#### implementation target:
...
## [ ] `Import-3` - `Phase 12 - Post-Accept Imported Reference Load Failure Research And Fix`
...
- treat the likely revoked-blob lifetime bug as the first hypothesis, but prove it before changing ownership rules
...
1. Reproduce the accepted staged-import `.glb` failure with one focused regression path and confirm the stored runtime error.
2. Audit how staged draft blob URLs flow through `commitStagedImportDraft()` and `closeStagedImportDraft()` in `src/app/store/useAppStore.ts`.
3. Fix accepted imported-reference asset ownership so closing the staged draft does not revoke the live asset path used by the committed row.
...
5. If needed, tighten the Browser-side error affordance just enough that a user can understand the failure and retry intentionally.
...
- [ ] reproduce the red-bar post-accept imported-reference failure with focused proof
- [ ] confirm whether accepted imported references still point at staged draft blob URLs after commit
- [ ] fix accepted asset lifetime so draft teardown does not break committed imports
- [ ] preserve honest Browser error and retry behavior for real load failures
- [ ] keep the pass narrow and avoid reopening the broader staged-import family
