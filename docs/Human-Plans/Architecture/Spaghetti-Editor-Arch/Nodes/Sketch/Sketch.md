...
55. 2026-03-22 10:58: Tightened `[3.2B-DrawSketch-3] Selection And Delete` so the first cut now explicitly includes blue `Window Selection` and green `Crossing Selection` box behavior, replacing the earlier single-select-only wording and making delete act on the resulting selection set
54. 2026-03-22 10:43: Re-focused `[3.2B-DrawSketch-3]` into `Selection And Delete`, so the old parent-bucket placeholder now reads as a real future phase for single committed-entity selection, viewport/list sync, and direct delete behavior instead of a broad mixed editing umbrella
53. 2026-03-22 10:33: Landed `[3.2B-DrawSketch-5] Circle Tool And Center-Radius Workflow`, so `Sketch Draw` now ships a real `Circle` / `cc` center-then-radius command with viewport-assisted `Center > Vec2` then `Radius > Float` console flow, live radius witness plus circle ghost preview, typed radius submission, and idle return after commit
52. 2026-03-22 10:19: Tightened `[3.2B-DrawSketch-5] Circle Tool And Center-Radius Workflow` again so the implementation contract now explicitly includes the viewport-assisted console flow, meaning stage 1 shows a live `Center > Vec2(N,N)` path driven by mouse hover, stage 2 shows `Radius > Float(N)` driven by center-to-cursor distance, and clicks at each stage act as the commit for center then radius
51. 2026-03-22 10:14: Re-locked `[3.2B-DrawSketch-5] Circle Tool And Center-Radius Workflow` so the primary typed path is now `cc > Vec2 > Float`, meaning the first circle cut is center-then-radius from the user’s point of view while still allowing viewport radius-witness picking and keeping the current internal `center + edge` component shape for the first implementation
50. 2026-03-22 10:09: Tightened `[3.2B-DrawSketch-5] Circle Tool And Center-Radius Workflow` into an implementation-ready spec by locking the exact typed flow as `cc > Vec2 > Vec2`, keeping the first committed circle as one first-class `center + edge` component, grounding the phase in the current draw-tool seam that still excludes active `circle`, and explicitly limiting the first cut to center-plus-edge commit with no diameter or tangent variants
49. 2026-03-22 08:35: Landed `[3.2B-DrawSketch-4] Rectangle Tool And Corner Workflow`, so `Sketch Draw` now ships a real `Rectangle` / `rec` two-point command with first-corner/opposite-corner commit, live viewport ghost preview, hybrid viewport-plus-console point entry, and idle return after the second accepted point
48. 2026-03-22 08:06: Tightened `[3.2B-DrawSketch-4] Rectangle Tool And Corner Workflow` into an implementation-ready spec by locking the exact typed flow as `rec > Vec2 > Vec2`, making second-point accept commit immediately with no extra finish step, grounding the phase in the current command/helper/runtime gaps, and explicitly limiting the first cut to axis-aligned first-corner/opposite-corner rectangles
47. 2026-03-22 07:58: Renamed the new post-`DrawSketch-3` follow-ons from letter suffixes to numeric `DrawSketch-N` ids, so the sketch plan now uses `[3.2B-DrawSketch-4] Rectangle Tool And Corner Workflow`, `[3.2B-DrawSketch-5] Circle Tool And Center-Radius Workflow`, and `[3.2B-DrawSketch-6] Endpoint Snap` instead of the temporary `3A / 3B / 3C` naming
46. 2026-03-22 07:47: Split the open `DrawSketch-3` bucket into concrete tool/snap follow-ons and added standalone future phase docs for `[3.2B-DrawSketch-3A] Rectangle Tool And Corner Workflow`, `[3.2B-DrawSketch-3B] Circle Tool And Center-Radius Workflow`, and `[3.2B-DrawSketch-3C] Endpoint Snap`, so the next AutoCAD-like sketch growth now has implementation-ready homes instead of staying inside one broad post-`Line` / `PLine` placeholder
...
Current shipped tool progression:
- `Line`
  - first point
  - second point
  - commit
- `PLine`
  ...
  - empty `Enter` finish once there are at least 2 committed points
- `Rectangle`
  - first corner
  - opposite corner
  - commit

Next tool-focused follow-ons:
- `[3.2B-DrawSketch-5]`
...
- later:
  - `Arc3Point`
  - `BezierSpline`
  - richer refinement passes once the first AutoCAD-like core tools are stable

#### `DrawSketch-3` - `Selection And Delete`

Purpose:
- add the first real committed-entity selection pass to `Sketch Draw` so the user can target existing sketch content and remove it directly inside the authoring surface

Owns:
- committed-entity selection
- entity-selection behavior inside idle `Sketch Draw`
- hover and selected feedback in the viewport
- blue `Window Selection` and green `Crossing Selection` box behavior
- selection sync with the `Entities` list
- direct delete of the current selection set
- the explicit separation between entity selection and later profile review
- the boundary before later grips, transforms, and broader edit tools

Implementation seam recommendation:
- keep selection state sketch-session-local:
  - one hover candidate id
  - one selected component-id set
  - one active selection-window draft
- let viewport picking and entity-row clicks resolve into the same selected committed component set
- treat committed-entity targeting as the default idle draw behavior of `Sketch Draw` instead of a peer tool
- keep profile review as its own separate surface/state
- clear selection when a new draw tool is armed so draw targeting and entity targeting do not compete
...
- `3.2B-DrawSketch-3` is now the later selection-and-delete follow-on for committed sketch entities
...
Purpose:
- add the first real `Rectangle` draw command to `Sketch Draw` using the same hybrid viewport-plus-console session model now proven by `Line` and `PLine`
...
#### [x] - `q1` Decide what first alias `Rectangle` should use inside `SketchDraw`.

##### Suggestion
- locked direction:
- keep the full command:
  - `rectangle`
- add the short local alias:
  - `rec`

#### [x] - `q2` Decide whether the first runtime cut should commit a first-class rectangle component or explode into four unrelated lines.

##### Suggestion
- locked direction:
- commit a first-class `rectangle` component
- do not explode the command into four unrelated line rows in the first cut

#### [x] - `q3` Decide the first typed command flow for `Rectangle`.

##### Suggestion
- locked direction:
- the first typed path is:
  - `rec > Vec2 > Vec2`
- first `Vec2`:
  - first corner
- second `Vec2`:
...
#### [x] - `q4` Decide whether `Rectangle` should require an extra confirm after the second point.

##### Suggestion
- locked direction:
- no
- the second accepted point should commit directly
- do not add an extra `Enter` or `Finish` step after `P2`

#### [x] - `q5` Decide what geometric model the first `Rectangle` cut should use.

##### Suggestion
- locked direction:
- axis-aligned in sketch space
- use:
  - first corner
  - opposite corner
- do not add:
  - rotated rectangles
  - 3-point rectangle variants
  - center-width-height variants

#### [x] - `q6` Decide what empty `Enter` should mean once `P1` exists.

##### Suggestion
- locked direction:
- empty `Enter` after `P1` accepts the current hovered `P2`
- typed second `Vec2` remains first-class and commits immediately
- do not use empty `Enter` as a separate post-`P2` finish step

### Implementation Spec

Purpose:
- ship the first honest AutoCAD-like rectangle primitive without widening into broader rectangle families or editing work

#### Landed Runtime Shape

- `runGeometrySketchDrawCommand(...)` now exposes:
  - `rectangle`
  - `rec`
- `useSpaghettiStore.ts` now commits one first-class `rectangle` component on the second accepted point or empty `Enter` against a hovered `P2`
- `GeometrySketchDrawHelper.ts` now renders live draft/ghost behavior for:
  - `rectangle`
- `geometrySketchOverlay.ts` now ghosts the full closed rectangle from the first corner plus hovered opposite corner
- `ViewportOverlay.tsx` and `ConsoleDock.tsx` now expose `Rectangle` in the active tool UI and two-point status flow

#### Scope

Owned here:
- toolbar exposure for:
  - `Rectangle`
- console commands for:
  - `rectangle`
  - `rec`
- rectangle draft state through the existing `geometrySketchSession`
- live four-edge ghost preview after `P1`
- immediate commit on second accepted point
- one first-class committed `rectangle` component with profile recompute

Not owned here:
- rotated rectangle modes
- center rectangle modes
- dimension-first rectangle grammar
- chamfer/fillet rectangle variants
- rectangle editing handles after commit
...
- semantic meaning:
  ...
  - second accepted point:
    - opposite corner
- mixed path remains valid:
  - click `P1`, type `P2`
  - type `P1`, click `P2`
  - click `P1`, hover `P2`, empty `Enter`

#### First Honest Runtime Target

- choose `Rectangle`
- place `P1` as the first corner
- place `P2` as the opposite corner
- preview the full four-edge rectangle while hovering `P2`
- commit one first-class rectangle component immediately on the second accepted point
- return to idle `Sketch Draw`
- point entry should stay hybrid:
  - viewport click
  - typed `Vec2`
  - mixed click/typed submission inside one active command
- command/status reads should stay explicit and short:
  - `G > S > SD > REC > P1 > Vec(N,N)`
  - `G > S > SD > REC > P2 > Vec(N,N)`

### Acceptance Checks

- `Rectangle` appears as a visible `Sketch Draw` tool
- typing `rectangle` or `rec` arms the same tool
- the typed happy path is:
  - `rec > Vec2 > Vec2`
- first accepted point becomes `P1`
- second accepted point becomes `P2` and commits immediately
- click and typed `Vec2` points can be mixed inside one rectangle command
- the viewport shows a full rectangle ghost after `P1` exists
- commit writes one first-class `rectangle` sketch component
- the committed rectangle is axis-aligned in sketch space using the two opposite corners
- the command returns to idle `Sketch Draw` after commit
