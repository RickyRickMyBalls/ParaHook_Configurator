...
## [x] Phase 4 - Dedicated Shadows Section Ownership Cleanup
...
- the toolbar now gives shadow-facing controls one explicit visible owner under a top-level `Shadows` section
- the global `Shadows` toggle moved out of the earlier view-facing section and into `Shadows`
- selected-light shadow controls moved out of `Environment` and now render in `Shadows` using the existing selected-light and shadow-control seams
- toolbar-local tab persistence was widened only enough to accept the new `shadows` top-level section key
- `Phase 5 - Final Environment-Surface Order And Wording Cleanup` is now the next remaining `Phase 2c` slice
...
Current status:
- `Phase 5` is intentionally skipped by user decision.
- the remaining work in this slice was optional visible wording and ordering cleanup only
- no additional runtime or state work landed for this slice
- `Phase 2c` is now treated as closed after `Phase 4`
- the ladder should move on to `Environment-1 / Phase 3 - Add Named Environment Presets And Preset Truth`
## Prep Completion - Implementation-Ready Handoff

`Environment-1 / Phase 2c` is now prepared as an implementation-ready visible-organization pass.

### Locked scope for implementation

- Keep the current startup scene as the locked default baseline.
- Keep scope only to `Environment-1 / Phase 2c`.
- Do not widen into later `Environment-1` preset/HDRI/runtime phases or `Environment-2`.
- Do not change environment runtime truth, lighting math, preset data truth, or Browser ownership boundaries.
- Keep `Environment` as the owner of active environment state.
- Keep `Catalog` as the owner of browseable HDRI assets, not active scene state.
- Keep `Browser` downstream from environment-owned light truth.

### Implementation cut sequence

Implement `Phase 2c` in exactly these remaining internal slices:

1. `Phase 4 - Dedicated Shadows Section Ownership Cleanup`
2. `Phase 5 - Final Environment-Surface Order And Wording Cleanup`

Do not start any later `Environment-1` work until both slices above are complete.

### Locked visible structure target

The implementation target for `Phase 2c` is the visible toolbar structure below:
...
- `Environment` keeps baseline scene and environment-facing controls only.
- `Shadows` becomes the explicit visible owner for shadow-facing controls.
- `Ground` remains separate and is not absorbed into `Environment` or `Shadows`.
...
Focused proof only if needed by the visible reorganization:

- `src/app/components/ViewToolbar.test.tsx`

Toolbar-local support only if a new top-level section key is required by the reorganization:

- `src/app/workspace/workspaceShellTypes.ts`
- `src/app/workspace/workspacePersistence.ts`

### Done condition for `Phase 2c`

`Phase 2c` is complete only when all of the following are true:

- shadow-facing controls have one explicit visible owner under `Shadows`
- final top-level order reads `Environment`, then `Shadows`, then `Ground`
- wording is cleaned up to match the reorganized ownership
- no environment runtime behavior changed
- no preset/HDRI/runtime work from later phases was started

### Prep outcome

- `Phase 2c` is implementation-ready.
- The next worker should implement `Phase 4` first, then `Phase 5`.
- No changelog entry belongs to this prep pass because no shipped runtime behavior changed.
