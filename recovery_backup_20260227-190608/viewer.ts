...
type ProfileAHandleId = "P1" | "P2" | "P3";
...
type ProfileAEditVisualOptions = {
  visible?: boolean;
  fillOpacity?: number;
  lineWidthPx?: number;
};
...
export class Viewer {
  ...
  private sceneGrid: THREE.Group;
  private sceneGridMajor: THREE.Group;
  private sceneGridDoubleMajor: THREE.Group;
  private sceneGridMaterials: Record<GridTier, Record<GridAxis, THREE.LineBasicMaterial>> | null = null;
  private gridExtentMm = 20000;
  private gridInfiniteEnabled = true;
  private gridMinorSpacingMm = 1;
  private gridMajorSpacingMm = 10;
  private gridDoubleMajorSpacingMm = 100;
  private gridMinorOpacity = 0.45;
  ...
  private gridLineColors: Record<GridTier, Record<GridAxis, string>> = {
    ...
    doubleMajor: { x: "#d6e6ff", y: "#9ab3d8" },
  };
  private gridColorPreset: "default" | "black" | "green_red" | "custom" = "default";
  private sceneGridVisible = false;
  ...
  private applyBackgroundMode() {
    ...
    if (this.sceneGridDoubleMajor) this.sceneGridDoubleMajor.visible = this.sceneGridVisible;
  }

  setGridVisible(enabled: boolean) {
    this.sceneGridVisible = !!enabled;
    if (this.sceneGrid) this.sceneGrid.visible = this.sceneGridVisible;
    if (this.sceneGridMajor) this.sceneGridMajor.visible = this.sceneGridVisible;
    if (this.sceneGridDoubleMajor) this.sceneGridDoubleMajor.visible = this.sceneGridVisible;
  }
  ...
  getGridVisible() {
    return this.sceneGridVisible;
  }

  setGridExtentMm(sizeMm: number) {
    const next = Number.isFinite(sizeMm) ? Math.round(Math.max(100, Math.min(20000, sizeMm))) : this.gridExtentMm;
    if (next === this.gridExtentMm) return;
    this.gridExtentMm = next;
    this.rebuildSceneGrid();
  }

  getGridExtentMm() {
    return this.gridExtentMm;
  }

  setGridInfiniteEnabled(enabled: boolean) {
    const next = !!enabled;
    if (next === this.gridInfiniteEnabled) return;
    this.gridInfiniteEnabled = next;
    this.updateGridFollowTargetPosition();
  }

  getGridInfiniteEnabled() {
    return this.gridInfiniteEnabled;
  }

  setGridCenterMode(mode: "origin" | "model") {
    const next = mode === "model" ? "model" : "origin";
    if (next === this.gridCenterMode) return;
    this.gridCenterMode = next;
    this.updateOriginAnchorPosition();
    this.updateGridFollowTargetPosition();
  }
  ...
  setGridMinorSpacingMm(mm: number) {
    const next = Number.isFinite(mm) ? Math.max(1, Math.min(100, mm)) : this.gridMinorSpacingMm;
    if (Math.abs(next - this.gridMinorSpacingMm) < 1e-6) return;
    this.gridMinorSpacingMm = next;
    if (this.gridMajorSpacingMm < this.gridMinorSpacingMm) {
      this.gridMajorSpacingMm = this.gridMinorSpacingMm;
    }
    if (this.gridDoubleMajorSpacingMm < this.gridMajorSpacingMm) {
      this.gridDoubleMajorSpacingMm = this.gridMajorSpacingMm;
    }
    this.rebuildSceneGrid();
  }

  getGridMinorSpacingMm() {
    return this.gridMinorSpacingMm;
  }

  setGridMajorSpacingMm(mm: number) {
    const next = Number.isFinite(mm) ? Math.max(1, Math.min(200, mm)) : this.gridMajorSpacingMm;
    const clamped = Math.max(next, this.gridMinorSpacingMm);
    if (Math.abs(clamped - this.gridMajorSpacingMm) < 1e-6) return;
    this.gridMajorSpacingMm = clamped;
    if (this.gridDoubleMajorSpacingMm < this.gridMajorSpacingMm) {
      this.gridDoubleMajorSpacingMm = this.gridMajorSpacingMm;
    }
    this.rebuildSceneGrid();
  ...
  getGridMajorSpacingMm() {
    return this.gridMajorSpacingMm;
  }

  setGridDoubleMajorSpacingMm(mm: number) {
    const next = Number.isFinite(mm) ? Math.max(1, Math.min(400, mm)) : this.gridDoubleMajorSpacingMm;
    const clamped = Math.max(next, this.gridMajorSpacingMm);
    if (Math.abs(clamped - this.gridDoubleMajorSpacingMm) < 1e-6) return;
    this.gridDoubleMajorSpacingMm = clamped;
    this.rebuildSceneGrid();
  }

  getGridDoubleMajorSpacingMm() {
    return this.gridDoubleMajorSpacingMm;
  }

  setGridMinorOpacity(opacity: number) {
    const next = Number.isFinite(opacity) ? Math.max(0, Math.min(1, opacity)) : this.gridMinorOpacity;
    this.gridMinorOpacity = next;
    this.applyGridMaterialOpacity();
  }

  getGridMinorOpacity() {
    return this.gridMinorOpacity;
  }

  setGridMajorOpacity(opacity: number) {
    const next = Number.isFinite(opacity) ? Math.max(0, Math.min(1, opacity)) : this.gridMajorOpacity;
    this.gridMajorOpacity = next;
    this.applyGridMaterialOpacity();
  }

  getGridMajorOpacity() {
    return this.gridMajorOpacity;
  }

  setGridDoubleMajorOpacity(opacity: number) {
    const next = Number.isFinite(opacity) ? Math.max(0, Math.min(1, opacity)) : this.gridDoubleMajorOpacity;
    this.gridDoubleMajorOpacity = next;
    this.applyGridMaterialOpacity();
  }

  getGridDoubleMajorOpacity() {
    return this.gridDoubleMajorOpacity;
  }

  // Backward compatibility wrappers for older UI paths.
  setGridSize(size: number) {
    this.setGridExtentMm(size);
  }

  getGridSize() {
    return this.getGridExtentMm();
  }

  setGridDivisions(divisions: number) {
    const div = Number.isFinite(divisions) ? Math.max(4, Math.min(1200, Math.round(divisions))) : 30;
    const spacing = Math.max(1, this.gridExtentMm / div);
    this.setGridMinorSpacingMm(spacing);
  }

  getGridDivisions() {
    return Math.max(1, Math.round(this.gridExtentMm / this.gridMinorSpacingMm));
  }

  setGridMajorStep(step: number) {
    const next = Number.isFinite(step) ? Math.max(1, Math.min(200, Math.round(step))) : 10;
    this.setGridMajorSpacingMm(this.gridMinorSpacingMm * next);
  }

  getGridMajorStep() {
    return Math.max(1, Math.round(this.gridMajorSpacingMm / Math.max(1e-6, this.gridMinorSpacingMm)));
  }

  setGridOpacity(opacity: number) {
    const next = Number.isFinite(opacity) ? Math.max(0, Math.min(1, opacity)) : this.gridMinorOpacity;
    this.setGridMinorOpacity(next);
    this.setGridMajorOpacity(Math.min(1, next * 1.3));
  }
  ...
  setGridLineColor(tier: GridTier, axis: GridAxis, hex: string) {
    ...
    if (mat) {
      mat.color.set(this.gridLineColors[tier][axis]);
      mat.needsUpdate = true;
    }
  }

  getGridLineColor(tier: GridTier, axis: GridAxis) {
    return this.gridLineColors[tier][axis];
  }

  setGridColorPreset(preset: "default" | "black" | "green_red") {
    const palette = this.getGridPresetColors(preset);
    this.gridLineColors.minor.x = palette.minor.x;
    this.gridLineColors.minor.y = palette.minor.y;
    this.gridLineColors.major.x = palette.major.x;
    this.gridLineColors.major.y = palette.major.y;
    this.gridLineColors.doubleMajor.x = palette.doubleMajor.x;
    this.gridLineColors.doubleMajor.y = palette.doubleMajor.y;
    this.gridColorPreset = preset;
    this.rebuildSceneGrid();
  }

  getGridColorPreset() {
    return this.gridColorPreset;
  }

  // Backward compatibility with removed high-contrast toggle.
  setGridHighContrast(enabled: boolean) {
    this.setGridColorPreset(enabled ? "green_red" : "default");
  }

  getGridHighContrast() {
    return this.gridColorPreset === "green_red";
  }

  private getGridPresetColors(preset: "default" | "black" | "green_red") {
    if (preset === "black") {
      return {
        minor: { x: "#000000", y: "#000000" },
        major: { x: "#000000", y: "#000000" },
        doubleMajor: { x: "#000000", y: "#000000" },
      };
    }
    if (preset === "green_red") {
      return {
        minor: { x: "#aa2a2a", y: "#1d7f3a" },
        major: { x: "#cf4040", y: "#2aa44b" },
        doubleMajor: { x: "#ff6666", y: "#4cd36f" },
      };
    }
    return {
      minor: { x: "#4b607f", y: "#2b3443" },
  ...
  private computeGridColorPresetFromCurrent(): "default" | "black" | "green_red" | "custom" {
    const matches = (preset: "default" | "black" | "green_red") => {
      const p = this.getGridPresetColors(preset);
      return this.gridLineColors.minor.x === p.minor.x
        && this.gridLineColors.minor.y === p.minor.y
        && this.gridLineColors.major.x === p.major.x
        && this.gridLineColors.major.y === p.major.y
        && this.gridLineColors.doubleMajor.x === p.doubleMajor.x
        && this.gridLineColors.doubleMajor.y === p.doubleMajor.y;
    };
    if (matches("default")) return "default";
    if (matches("black")) return "black";
    if (matches("green_red")) return "green_red";
    return "custom";
  }

  private applyGridMaterialOpacity() {
    const applyOpacity = (tier: GridTier, opacity: number) => {
      const mats = this.sceneGridMaterials?.[tier];
      if (!mats) return;
    ...
    applyOpacity("minor", this.gridMinorOpacity);
    applyOpacity("major", this.gridMajorOpacity);
    applyOpacity("doubleMajor", this.gridDoubleMajorOpacity);
  }

  private getModelCenterWorld(out = new THREE.Vector3()) {
    const mesh = this.mesh;
    if (!mesh || !mesh.geometry) return out.set(0, 0, 0);
    const geo = mesh.geometry;
    if (!geo.boundingSphere) geo.computeBoundingSphere();
    const center = geo.boundingSphere?.center;
    if (!center) return out.set(0, 0, 0);
    return out.copy(center).applyMatrix4(mesh.matrixWorld);
  }
  ...
  private updateOriginAnchorPosition() {
    // Keep origin gizmo fixed at absolute world origin even when grid is model-centered.
    if (this.originAxes) this.originAxes.position.set(0, 0, 0);
    if (this.originDot) this.originDot.position.set(0, 0, 0);
  }

  private updateGridFollowTargetPosition() {
    if (!this.sceneGrid || !this.sceneGridMajor || !this.sceneGridDoubleMajor) return;
    const anchor = this.getGridAnchorPoint(new THREE.Vector3());
    if (!this.gridInfiniteEnabled) {
      this.sceneGrid.position.copy(anchor);
      this.sceneGridMajor.position.copy(anchor);
      this.sceneGridDoubleMajor.position.copy(anchor);
      return;
    }
    const target = this.getControlTarget();
    const snap = Math.max(1, this.gridMinorSpacingMm);
    const snapPos = (v: number, base: number) => Math.round((v - base) / snap) * snap + base;
    const x = snapPos(target.x, anchor.x);
    const y = snapPos(target.y, anchor.y);
    this.sceneGrid.position.set(x, y, anchor.z);
    this.sceneGridMajor.position.set(x, y, anchor.z);
    this.sceneGridDoubleMajor.position.set(x, y, anchor.z);
  }

  private buildGridTierLines(
    tier: GridTier,
    extent: number,
    divisions: number
  ): { group: THREE.Group; mats: Record<GridAxis, THREE.LineBasicMaterial> } {
    const safeDiv = Math.max(1, Math.round(divisions));
    const half = extent * 0.5;
    const step = extent / safeDiv;
    ...
    xGeom.setAttribute("position", new THREE.Float32BufferAttribute(xPositions, 3));
    const yGeom = new THREE.BufferGeometry();
    yGeom.setAttribute("position", new THREE.Float32BufferAttribute(yPositions, 3));
    const xMat = new THREE.LineBasicMaterial({
      color: new THREE.Color(this.gridLineColors[tier].x),
      transparent: true,
      opacity: 1,
      depthWrite: false,
    });
    const yMat = new THREE.LineBasicMaterial({
      color: new THREE.Color(this.gridLineColors[tier].y),
      transparent: true,
      opacity: 1,
      depthWrite: false,
    });
    const xLines = new THREE.LineSegments(xGeom, xMat);
    const yLines = new THREE.LineSegments(yGeom, yMat);
    const group = new THREE.Group();
    group.add(xLines);
    group.add(yLines);
    return { group, mats: { x: xMat, y: yMat } };
  }

  private disposeGridGroup(group: THREE.Group | undefined | null) {
    if (!group) return;
    group.traverse((obj) => {
  ...
  private rebuildSceneGrid() {
    this.disposeGridGroup(this.sceneGrid);
    this.disposeGridGroup(this.sceneGridMajor);
    this.disposeGridGroup(this.sceneGridDoubleMajor);
    ...
    if (this.sceneGridDoubleMajor?.parent) this.sceneGridDoubleMajor.parent.remove(this.sceneGridDoubleMajor);
    const maxDivisions = 1200;
    const safeMinor = Math.max(1, this.gridMinorSpacingMm);
    const safeMajor = Math.max(safeMinor, this.gridMajorSpacingMm);
    const safeDoubleMajor = Math.max(safeMajor, this.gridDoubleMajorSpacingMm);
    const desiredMinorDiv = Math.max(1, Math.round(this.gridExtentMm / safeMinor));
    const desiredMajorDiv = Math.max(1, Math.round(this.gridExtentMm / safeMajor));
    const desiredDoubleMajorDiv = Math.max(1, Math.round(this.gridExtentMm / safeDoubleMajor));
    const minorDivisions = Math.max(1, Math.min(maxDivisions, desiredMinorDiv));
    const majorDivisions = Math.max(1, Math.min(maxDivisions, desiredMajorDiv));
    const doubleMajorDivisions = Math.max(1, Math.min(maxDivisions, desiredDoubleMajorDiv));
    const minorExtent = Math.max(this.gridExtentMm, safeMinor * minorDivisions);
    const majorExtent = Math.max(this.gridExtentMm, safeMajor * majorDivisions);
    const doubleMajorExtent = Math.max(this.gridExtentMm, safeDoubleMajor * doubleMajorDivisions);

    const minor = this.buildGridTierLines("minor", minorExtent, minorDivisions);
    const major = this.buildGridTierLines("major", majorExtent, majorDivisions);
    const doubleMajor = this.buildGridTierLines("doubleMajor", doubleMajorExtent, doubleMajorDivisions);
    this.sceneGrid = minor.group;
    this.sceneGridMajor = major.group;
    this.sceneGridDoubleMajor = doubleMajor.group;
    this.sceneGridMaterials = {
      minor: minor.mats,
      major: major.mats,
      doubleMajor: doubleMajor.mats,
    };
    this.applyGridMaterialOpacity();
    this.scene.add(this.sceneGrid);
    this.scene.add(this.sceneGridMajor);
    this.scene.add(this.sceneGridDoubleMajor);
    this.updateOriginAnchorPosition();
    this.updateGridFollowTargetPosition();
    this.applyBackgroundMode();
  }
  ...
  private profileAEditGroup: THREE.Group;
  private profileAEditFillMesh: THREE.Mesh;
  private profileAEditFillMat: THREE.MeshBasicMaterial;
  private profileAEditOutline: Line2;
  private profileAEditOutlineGeo: LineGeometry;
  private profileAEditOutlineMat: LineMaterial;
  private profileAEditEnabled = false;
  private profileADragFrame:
    | {
    ...
    | null = null;
  private profileAHandleWorldById = new Map<ProfileAHandleId, THREE.Vector3>();
  private profileAHandleIdByMesh = new Map<THREE.Object3D, ProfileAHandleId>();
  ...
  private profileADragState:
    | {
        pointerId: number;
        handleId: ProfileAHandleId;
        plane: THREE.Plane;
      }
    | null = null;
  private onProfileAHandleDragCommitCb:
    | ((payload: {
        id: ProfileAHandleId;
  ...
  private refreshProfileAHandleMeshMap() {
    this.profileAHandleIdByMesh.clear();
    this.profileAHandleMeshById.clear();
    const ids: ProfileAHandleId[] = ["P1", "P2", "P3"];
    for (let i = 0; i < Math.min(ids.length, this.aArcMeshes.length); i++) {
      const id = ids[i];
      const mesh = this.aArcMeshes[i];
      this.profileAHandleIdByMesh.set(mesh, id);
      this.profileAHandleMeshById.set(id, mesh);
    }
  }
  ...
  private toProfileAWorldFromLocal(local: { u: number; z: number }) {
    if (!this.profileADragFrame) return null;
    return this.profileADragFrame.origin
      .clone()
      .addScaledVector(this.profileADragFrame.uAxis, local.u)
      .addScaledVector(this.profileADragFrame.zAxis, local.z);
  }

  private toProfileALocalFromWorld(world: THREE.Vector3) {
    if (!this.profileADragFrame) return null;
    const rel = world.clone().sub(this.profileADragFrame.origin);
    return {
      u: rel.dot(this.profileADragFrame.uAxis),
      z: rel.dot(this.profileADragFrame.zAxis),
    };
  }

  private computeProfileAOverlayWorldFromHandles() {
    const p1 = this.profileAHandleWorldById.get("P1");
    const p2 = this.profileAHandleWorldById.get("P2");
    const p3 = this.profileAHandleWorldById.get("P3");
    if (!p1 || !p2 || !p3) return null;
    const l1 = this.toProfileALocalFromWorld(p1);
    const l2 = this.toProfileALocalFromWorld(p2);
    const l3 = this.toProfileALocalFromWorld(p3);
    if (!l1 || !l2 || !l3) return null;

    const steps = 48;
    const points: XYZ[] = [];
    const p0 = { u: 0, z: 0 };
    const bez3 = (t: number, a: { u: number; z: number }, b: { u: number; z: number }, c: { u: number; z: number }, d: { u: number; z: number }) => {
      const mt = 1 - t;
      return {
        u: mt * mt * mt * a.u + 3 * mt * mt * t * b.u + 3 * mt * t * t * c.u + t * t * t * d.u,
        z: mt * mt * mt * a.z + 3 * mt * mt * t * b.z + 3 * mt * t * t * c.z + t * t * t * d.z,
      };
    };
    for (let i = 0; i <= steps; i++) {
      const t = i / steps;
      const p = bez3(t, p0, l1, l2, l3);
      const w = this.toProfileAWorldFromLocal(p);
      if (!w) continue;
      points.push({ x: w.x, y: w.y, z: w.z });
    }
    const w0 = this.toProfileAWorldFromLocal({ u: 0, z: 0 });
    if (!w0 || points.length < 3) return null;
    points.push({ x: w0.x, y: w0.y, z: w0.z });
    return points;
  }

  private setProfileAEditVisualFromWorldPoints(pointsWorld: XYZ[], options: ProfileAEditVisualOptions = {}) {
    const visible = options.visible ?? true;
    this.profileAEditFillMat.opacity = THREE.MathUtils.clamp(options.fillOpacity ?? this.profileAEditFillMat.opacity, 0, 1);
    this.profileAEditOutlineMat.linewidth = Math.max(1, options.lineWidthPx ?? this.profileAEditOutlineMat.linewidth);
    if (
      !visible ||
      pointsWorld.length < 3 ||
      pointsWorld.some((p) => !Number.isFinite(p.x) || !Number.isFinite(p.y) || !Number.isFinite(p.z))
    ) {
      this.profileAEditFillMesh.visible = false;
      this.profileAEditOutline.visible = false;
      return;
    }

    const worldVec = pointsWorld.map((p) => new THREE.Vector3(p.x, p.y, p.z));
    const first = worldVec[0];
    const second = worldVec[1] ?? first;
    let uAxis = second.clone().sub(first);
    if (uAxis.lengthSq() < 1e-9) uAxis.set(1, 0, 0);
    uAxis.normalize();
    let normal = new THREE.Vector3();
    for (let i = 2; i < worldVec.length; i++) {
      const c = worldVec[i].clone().sub(first);
      normal = uAxis.clone().cross(c);
      if (normal.lengthSq() > 1e-10) break;
    }
    if (normal.lengthSq() < 1e-10) {
      normal = this.profileADragFrame?.normal.clone() ?? new THREE.Vector3(0, 1, 0);
    }
    normal.normalize();
    const vAxis = normal.clone().cross(uAxis).normalize();
    const points2D = worldVec.map((p) => {
      const rel = p.clone().sub(first);
      return new THREE.Vector2(rel.dot(uAxis), rel.dot(vAxis));
    });
    if (points2D[0].distanceTo(points2D[points2D.length - 1]) > 1e-5) points2D.push(points2D[0].clone());

    let fillGeo: THREE.ShapeGeometry;
    try {
    ...
    const pos = fillGeo.getAttribute("position");
    for (let i = 0; i < pos.count; i++) {
      const x = pos.getX(i);
      const y = pos.getY(i);
      const world = first.clone().addScaledVector(uAxis, x).addScaledVector(vAxis, y);
      pos.setXYZ(i, world.x, world.y, world.z);
    }
    pos.needsUpdate = true;
    fillGeo.computeVertexNormals();
    const prevFillGeo = this.profileAEditFillMesh.geometry;
    this.profileAEditFillMesh.geometry = fillGeo;
    prevFillGeo.dispose();
    this.profileAEditFillMesh.visible = true;

    const linePositions: number[] = [];
    for (const p of worldVec) linePositions.push(p.x, p.y, p.z);
    if (linePositions.length >= 6) {
      this.profileAEditOutlineGeo.setPositions(linePositions);
      this.profileAEditOutline.computeLineDistances();
      this.profileAEditOutline.visible = true;
    } else {
      this.profileAEditOutline.visible = false;
    }
  }
  ...
  setAArcVizVisible(v: boolean) {
    ...
    if (!this.aArcVisible) this.cancelProfileADrag(false);
    this.updateProfileAEditVisibility();
  }
  ...
  setAArcPoints(points: XYZ[]) {
    ...
    this.refreshProfileAHandleMeshMap();
    const ids: ProfileAHandleId[] = ["P1", "P2", "P3"];
    for (let i = 0; i < Math.min(ids.length, points.length); i++) {
      const p = points[i];
      this.profileAHandleWorldById.set(ids[i], new THREE.Vector3(p.x, p.y, p.z));
    }
    if (this.profileAEditEnabled) {
      const overlay = this.computeProfileAOverlayWorldFromHandles();
      if (overlay) this.setProfileAEditVisualFromWorldPoints(overlay);
    }
  }
  ...
  setProfileAEditVisual(pointsWorld: XYZ[], options: ProfileAEditVisualOptions = {}) {
    this.setProfileAEditVisualFromWorldPoints(pointsWorld, options);
    this.updateProfileAEditVisibility();
  }

  setProfileAEditEnabled(enabled: boolean) {
    this.profileAEditEnabled = !!enabled;
    if (!this.profileAEditEnabled) this.cancelProfileADrag(false);
    this.updateProfileAEditVisibility();
  }

  setProfileAEditDragFrame(frame: { origin: XYZ; uAxis: XYZ; zAxis: XYZ }) {
    ...
    normal.normalize();
    this.profileADragFrame = { origin, uAxis, zAxis, normal };
  }

  setProfileAEditHandleMetadata(handles: Array<{ id: ProfileAHandleId; world: XYZ }>) {
    for (const h of handles) {
      const world = new THREE.Vector3(h.world.x, h.world.y, h.world.z);
      this.profileAHandleWorldById.set(h.id, world);
      const mesh = this.profileAHandleMeshById.get(h.id);
      if (mesh) mesh.position.copy(world);
    }
    if (this.profileAEditEnabled) {
      const overlay = this.computeProfileAOverlayWorldFromHandles();
      if (overlay) this.setProfileAEditVisualFromWorldPoints(overlay);
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.sceneGrid = new THREE.Group();
    this.sceneGridMajor = new THREE.Group();
    this.sceneGridDoubleMajor = new THREE.Group();
    this.scene.add(this.sceneGrid);
    this.scene.add(this.sceneGridMajor);
    this.scene.add(this.sceneGridDoubleMajor);
    this.rebuildSceneGrid();
    ...
    this.profileAEditGroup = new THREE.Group();
    this.profileAEditGroup.name = "profileAEditGroup";
    this.profileAEditGroup.visible = false;
    this.arcVizGroup.add(this.profileAEditGroup);
    this.profileAEditFillMat = new THREE.MeshBasicMaterial({
      color: 0xffd34d,
      transparent: true,
      opacity: 0.5,
    ...
    this.profileAEditFillMesh = new THREE.Mesh(new THREE.BufferGeometry(), this.profileAEditFillMat);
    this.profileAEditFillMesh.name = "profileAEditFill";
    this.profileAEditFillMesh.renderOrder = 55;
    this.profileAEditGroup.add(this.profileAEditFillMesh);
    this.profileAEditOutlineGeo = new LineGeometry();
    this.profileAEditOutlineMat = new LineMaterial({
      color: 0xfff2a8,
      linewidth: 4,
      transparent: true,
      opacity: 0.95,
  ...
  resize() {
    ...
    this.profileAEditOutlineMat.resolution.set(Math.max(1, w), Math.max(1, h));
  }
  ...
  snapToLeftView() {
    this.snapCameraToDirection(new THREE.Vector3(-1, 0, 0));
  }
  ...
  private getCanvasPointerNdc(e: PointerEvent) {
    const rect = this.canvas.getBoundingClientRect();
    const w = this.canvas.clientWidth || 1;
    const h = this.canvas.clientHeight || 1;
    const px = e.clientX - rect.left;
    ...
    return {
      px,
      py,
      ndc: new THREE.Vector2((px / w) * 2 - 1, -((py / h) * 2 - 1)),
    };
  }

  private updateProfileAOverlayFromCurrentHandlePoints() {
    if (!this.profileAEditEnabled) return;
    const overlay = this.computeProfileAOverlayWorldFromHandles();
    if (overlay) this.setProfileAEditVisualFromWorldPoints(overlay);
  }

  private tryStartProfileADrag(e: PointerEvent) {
    if (e.button !== 0) return false;
    if (!this.profileAEditEnabled || !this.aArcVisible || !this.profileADragFrame) return false;
    if (this.profileAHandleMeshById.size === 0) return false;

    const { ndc } = this.getCanvasPointerNdc(e);
    this.profileAEditPointerNdc.copy(ndc);
    this.profileAEditRaycaster.setFromCamera(this.profileAEditPointerNdc, this.camera);
    const handles = Array.from(this.profileAHandleMeshById.values());
    const hits = this.profileAEditRaycaster.intersectObjects(handles, false);
    const hit = hits[0];
    if (!hit) return false;
    const handleId = this.profileAHandleIdByMesh.get(hit.object);
    if (!handleId) return false;

    const plane = new THREE.Plane().setFromNormalAndCoplanarPoint(this.profileADragFrame.normal, this.profileADragFrame.origin);
    this.profileADragState = {
  ...
  private updateProfileADrag(e: PointerEvent) {
    if (!this.profileADragState || e.pointerId !== this.profileADragState.pointerId || !this.profileADragFrame) return;
    const { ndc } = this.getCanvasPointerNdc(e);
    this.profileAEditPointerNdc.copy(ndc);
    this.profileAEditRaycaster.setFromCamera(this.profileAEditPointerNdc, this.camera);
    const hitPoint = new THREE.Vector3();
    if (!this.profileAEditRaycaster.ray.intersectPlane(this.profileADragState.plane, hitPoint)) return;

    const local = this.toProfileALocalFromWorld(hitPoint);
    if (!local) return;
    const clampedWorld = this.toProfileAWorldFromLocal(local);
    if (!clampedWorld) return;
    this.profileAHandleWorldById.set(this.profileADragState.handleId, clampedWorld.clone());
  ...
  private finishProfileADrag(commit: boolean) {
    const drag = this.profileADragState;
    if (!drag) return;
    ...
    const getLocal = (id: ProfileAHandleId) => {
      const w = this.profileAHandleWorldById.get(id);
      if (!w) return { u: 0, z: 0 };
      return this.toProfileALocalFromWorld(w) ?? { u: 0, z: 0 };
    };
    const allLocal: Record<ProfileAHandleId, { u: number; z: number }> = {
      P1: getLocal("P1"),
      P2: getLocal("P2"),
      P3: getLocal("P3"),
    };
    this.onProfileAHandleDragCommitCb({
      id: drag.handleId,
      local: allLocal[drag.handleId],
      allLocal,
    });
  }

  private cancelProfileADrag(commit: boolean) {
    if (!this.profileADragState) return;
    const pointerId = this.profileADragState.pointerId;
    try {
      this.canvas.releasePointerCapture(pointerId);
    } catch {}
    this.finishProfileADrag(commit);
  }

  private onProfileADragPointerMove = (e: PointerEvent) => {
    if (!this.profileADragState) return;
    this.updateProfileADrag(e);
    e.preventDefault();
    e.stopPropagation();
  };

  private onProfileADragPointerUp = (e: PointerEvent) => {
    if (!this.profileADragState || e.pointerId !== this.profileADragState.pointerId) return;
    this.cancelProfileADrag(true);
    e.preventDefault();
    e.stopPropagation();
  };

  private onProfileADragPointerCancel = (e: PointerEvent) => {
    if (!this.profileADragState || e.pointerId !== this.profileADragState.pointerId) return;
    this.cancelProfileADrag(false);
    e.preventDefault();
    e.stopPropagation();
  };
  ...
  private onCanvasPointerDown = (e: PointerEvent) => {
    ...
    const insideAxisVp = px >= vp.left && px <= vp.left + vp.size && py >= vp.top && py <= vp.top + vp.size;
    if (this.axisGizmoEnabled && insideAxisVp) {
      this.updateAxisGizmoCameraFromMainCamera();
      this.axisPointerNdc.set(((px - vp.left) / vp.size) * 2 - 1, -(((py - vp.top) / vp.size) * 2 - 1));
      this.axisRaycaster.setFromCamera(this.axisPointerNdc, this.axisCamera);
