...
const bootLoadingSmoothProgressFillEl = document.getElementById("bootLoadingSmoothProgressFill") as HTMLDivElement | null;
const bootLoadingSmoothProgressTextEl = document.getElementById("bootLoadingSmoothProgressText") as HTMLDivElement | null;
...
let bootSmoothTargetPct = 0;
let bootSmoothDisplayedPct = 0;
let bootSmoothRaf = 0;
let bootSmoothLastTs = 0;
...
function renderBootLoadingOverlay() {
  ...
  const smoothPct = Math.max(0, Math.min(100, Math.round(bootSmoothDisplayedPct)));
  ...
  if (bootLoadingSmoothProgressFillEl) {
    bootLoadingSmoothProgressFillEl.style.width = `${smoothPct}%`;
  }
  if (bootLoadingSmoothProgressTextEl) {
    bootLoadingSmoothProgressTextEl.textContent = `${smoothPct}%`;
  }
...
function tickBootSmoothProgress(ts: number) {
  if (!bootLoadingActive) {
    bootSmoothRaf = 0;
    bootSmoothLastTs = 0;
    return;
  }
  const dt = bootSmoothLastTs > 0 ? Math.max(0, Math.min(100, ts - bootSmoothLastTs)) : 16;
  bootSmoothLastTs = ts;
  const target = Math.max(bootSmoothDisplayedPct, bootSmoothTargetPct);
  const diff = target - bootSmoothDisplayedPct;
  if (Math.abs(diff) < 0.01) {
    bootSmoothDisplayedPct = target;
  } else {
    const gain = Math.min(1, dt * 0.0125);
    const minStep = (dt / 1000) * 8;
    const step = Math.max(minStep, Math.abs(diff) * gain);
    bootSmoothDisplayedPct += Math.sign(diff) * Math.min(Math.abs(diff), step);
  }
  renderBootLoadingOverlay();
  bootSmoothRaf = window.requestAnimationFrame(tickBootSmoothProgress);
}

function ensureBootSmoothProgressAnimator() {
  if (!bootLoadingActive || bootSmoothRaf) return;
  bootSmoothLastTs = 0;
  bootSmoothRaf = window.requestAnimationFrame(tickBootSmoothProgress);
}
...
function reportBootRuntimeError(message: string) {
  const text = String(message || "unknown startup error");
  bootStatusDetail = `main error: ${text}`;
  pushBootLoadingLogLine(`error: ${text}`);
  setBootProgressPct(100);
  bootSmoothTargetPct = 100;
  bootSmoothDisplayedPct = 100;
  renderBootLoadingOverlay();
}

window.addEventListener("error", (ev) => {
  reportBootRuntimeError(ev.message || "window error");
});
window.addEventListener("unhandledrejection", (ev) => {
  const reason = (ev as PromiseRejectionEvent).reason;
  const msg = reason instanceof Error ? reason.message : String(reason ?? "promise rejection");
  reportBootRuntimeError(msg);
});

function setBootProgressPct(nextPct: number) {
  bootProgressPct = Math.max(bootProgressPct, Math.min(100, nextPct));
  bootSmoothTargetPct = Math.max(bootSmoothTargetPct, bootProgressPct);
  ensureBootSmoothProgressAnimator();
  renderBootLoadingOverlay();
}
...
const gizmoGridPresetInfiniteBtnEl = document.getElementById("gizmoGridPresetInfiniteBtn") as HTMLButtonElement | null;
...
const gizmoGridMinorQuickRowEl = document.getElementById("gizmoGridMinorQuickRow") as HTMLDivElement | null;
const gizmoGridDoubleMajorSpacingRangeEl = document.getElementById("gizmoGridDoubleMajorSpacingRange") as HTMLInputElement | null;
const gizmoGridDoubleMajorPresetRowEl = document.getElementById("gizmoGridDoubleMajorPresetRow") as HTMLDivElement | null;
const gizmoGridMajorOpacityRangeEl = document.getElementById("gizmoGridMajorOpacityRange") as HTMLInputElement | null;
const gizmoGridMinorOpacityRangeEl = document.getElementById("gizmoGridMinorOpacityRange") as HTMLInputElement | null;
const gizmoGridDoubleMajorOpacityRangeEl = document.getElementById("gizmoGridDoubleMajorOpacityRange") as HTMLInputElement | null;
const gizmoGridColorPresetDefaultBtnEl = document.getElementById("gizmoGridColorPresetDefaultBtn") as HTMLButtonElement | null;
const gizmoGridColorPresetBlackBtnEl = document.getElementById("gizmoGridColorPresetBlackBtn") as HTMLButtonElement | null;
const gizmoGridColorPresetGreenRedBtnEl = document.getElementById("gizmoGridColorPresetGreenRedBtn") as HTMLButtonElement | null;
const gizmoGridColorDoubleMajorXEl = document.getElementById("gizmoGridColorDoubleMajorX") as HTMLInputElement | null;
const gizmoGridColorDoubleMajorYEl = document.getElementById("gizmoGridColorDoubleMajorY") as HTMLInputElement | null;
const gizmoGridColorMajorXEl = document.getElementById("gizmoGridColorMajorX") as HTMLInputElement | null;
const gizmoGridColorMajorYEl = document.getElementById("gizmoGridColorMajorY") as HTMLInputElement | null;
const gizmoGridColorMinorXEl = document.getElementById("gizmoGridColorMinorX") as HTMLInputElement | null;
const gizmoGridColorMinorYEl = document.getElementById("gizmoGridColorMinorY") as HTMLInputElement | null;
...
const gizmoUseMobileCollapsedStyleEl = document.getElementById("gizmoUseMobileCollapsedStyle") as HTMLInputElement | null;
type GizmoToolbarState = "expanded" | "collapsedHorizontal" | "collapsedVertical";
type GizmoExpandedPanel = "gizmo" | "view" | "controls" | null;
let useMobileCollapsedStyle = true;
...
let gridColorSnapshotBeforeWhite: {
  doubleMajor: { x: string; y: string };
  major: { x: string; y: string };
  minor: { x: string; y: string };
} | null = null;

function clampGizmoExpandedSize(size: number) {
  return Math.max(132, Math.round(size));
}

function clampGizmoCompactSize(size: number, expandedSize: number) {
  return Math.max(72, Math.min(clampGizmoExpandedSize(expandedSize), Math.round(size)));
}

function getLayoutDefaultCollapsedState(isMobile: boolean): GizmoToolbarState {
  return isMobile ? "collapsedVertical" : "collapsedHorizontal";
}

function getPreferredCollapsedToolbarState(): GizmoToolbarState {
  return useMobileCollapsedStyle ? "collapsedVertical" : "collapsedHorizontal";
}

function isExpandedToolbarState() {
  return gizmoToolbarState === "expanded";
}

function isExpandedPanelOpen(panel: Exclude<GizmoExpandedPanel, null>) {
  return isExpandedToolbarState() && activeExpandedPanel === panel;
}

function getTargetGizmoViewportSize(nextState: GizmoToolbarState, isMobile: boolean) {
  if (nextState === "expanded") {
    return isMobile ? mobileExpandedGizmoSize : desktopExpandedGizmoSize;
  }
  if (isMobile) {
    return mobileCollapsedGizmoSize;
  }
  return desktopExpandedGizmoSize;
}

function captureCurrentGizmoSizeForState() {
  const current = Math.max(72, Math.round(viewer.getAxisGizmoViewportSize?.() ?? 300));
  if (isMobileLayoutActive) {
    if (isExpandedToolbarState()) {
      mobileExpandedGizmoSize = clampGizmoExpandedSize(current);
      mobileCollapsedGizmoSize = clampGizmoCompactSize(Math.round(current * 0.5), mobileExpandedGizmoSize);
      return;
    }
    mobileCollapsedGizmoSize = clampGizmoCompactSize(current, mobileExpandedGizmoSize);
    return;
  }
  desktopExpandedGizmoSize = clampGizmoExpandedSize(current);
}

function isGizmoToolbarCollapsed() {
  return !isExpandedToolbarState();
}

function syncGizmoToolbarButtonLabels() {
  if (!gizmoToggleBtnEl) return;
  gizmoToggleBtnEl.textContent = "Gizmo";
  gizmoToggleBtnEl.title = isGizmoToolbarCollapsed() ? "Expand gizmo toolbar" : "Gizmo settings";
  gizmoToggleBtnEl.setAttribute("aria-label", isGizmoToolbarCollapsed() ? "Expand gizmo toolbar" : "Gizmo settings");
  if (gizmoViewBtnEl) gizmoViewBtnEl.textContent = "View";
  if (gizmoControlsBtnEl) gizmoControlsBtnEl.textContent = "Controls";
}

function setActiveExpandedPanel(panel: GizmoExpandedPanel) {
  activeExpandedPanel = panel;
  gizmoViewPanelEl?.classList.toggle("open", isExpandedPanelOpen("view"));
}

function setToolbarState(next: GizmoToolbarState, panel: GizmoExpandedPanel = activeExpandedPanel) {
  captureCurrentGizmoSizeForState();
  const prev = gizmoToolbarState;
  gizmoToolbarState = next;
  const collapsedVertical = next === "collapsedVertical";
  const collapsedHorizontal = next === "collapsedHorizontal";
  document.body.classList.toggle("gizmo-collapsed-vertical", collapsedVertical);
  document.body.classList.toggle("gizmo-collapsed-horizontal", collapsedHorizontal);

  viewer.setAxisGizmoEnabled?.(true);
  if (isExpandedToolbarState()) {
    setActiveExpandedPanel(panel ?? null);
  } else {
    setActiveExpandedPanel(null);
  }
  if (prev !== next) {
    viewer.setAxisGizmoViewportSize(getTargetGizmoViewportSize(next, isMobileLayoutActive));
  }

  syncGizmoToolbarButtonLabels();
  positionGizmoViewportResizeHandle();
}

function openPanel(panel: Exclude<GizmoExpandedPanel, null>) {
  setToolbarState("expanded", panel);
}

function collapseToPreferredStyle() {
  setToolbarState(getPreferredCollapsedToolbarState(), null);
}

function syncCollapsedStylePreferenceFromUI() {
  useMobileCollapsedStyle = gizmoUseMobileCollapsedStyleEl ? !!gizmoUseMobileCollapsedStyleEl.checked : true;
  if (isGizmoToolbarCollapsed()) {
    setToolbarState(getPreferredCollapsedToolbarState(), null);
    return;
  }
  syncGizmoToolbarButtonLabels();
...
if (gizmoUseMobileCollapsedStyleEl) {
  gizmoUseMobileCollapsedStyleEl.checked = isMobileLayoutActive;
  gizmoUseMobileCollapsedStyleEl.addEventListener("change", syncCollapsedStylePreferenceFromUI);
}
...
function positionGizmoViewportResizeHandle() {
  ...
  if (!vp) return;
  const gizmoPanelOpen = isExpandedPanelOpen("gizmo");
  const viewPanelOpen = isExpandedPanelOpen("view");
  const controlsPanelOpen = isExpandedPanelOpen("controls");
  const collapsedMode: "horizontal" | "vertical" | null =
    gizmoToolbarState === "collapsedVertical"
      ? "vertical"
      : gizmoToolbarState === "collapsedHorizontal"
        ? "horizontal"
        : null;
  const state = {
    gizmoOpen: gizmoPanelOpen,
    gizmoPanelHeight: gizmoPanelOpen && gizmoGizmoPanelEl ? gizmoGizmoPanelEl.getBoundingClientRect().height : 0,
    viewOpen: viewPanelOpen,
    controlsOpen: controlsPanelOpen,
    collapsedMode,
    spinEnabled: !!viewer.getAutoSpinEnabled?.(),
    viewPanelHeight: viewPanelOpen && gizmoViewPanelEl ? gizmoViewPanelEl.getBoundingClientRect().height : 0,
...
if (gizmoToggleBtnEl) {
  gizmoToggleBtnEl.addEventListener("click", (e) => {
    ...
    if (activeExpandedPanel === "gizmo") {
      collapseToPreferredStyle();
      return;
    }
    openPanel("gizmo");
  });
...
if (gizmoSpinToggleBtnEl) {
  const syncSpinBtn = () => {
    ...
    if (gizmoSpinSpeedRangeEl) {
      gizmoSpinSpeedRangeEl.style.display = isExpandedPanelOpen("controls") && active ? "block" : "none";
    }
    if (gizmoSpinSpeedLabelEl) {
      gizmoSpinSpeedLabelEl.style.display = isExpandedPanelOpen("controls") && active ? "block" : "none";
    }
  };
...
if (gizmoControlsBtnEl) {
  gizmoControlsBtnEl.addEventListener("click", (e) => {
    ...
    if (activeExpandedPanel === "controls") {
      collapseToPreferredStyle();
      return;
    }
    openPanel("controls");
...
const syncGridPresetButtons = () => {
  gizmoGridPresetInfiniteBtnEl?.setAttribute("aria-pressed", gridSizePreset === "infinite" ? "true" : "false");
  gizmoGridPreset300BtnEl?.setAttribute("aria-pressed", gridSizePreset === "300" ? "true" : "false");
  gizmoGridPreset250BtnEl?.setAttribute("aria-pressed", gridSizePreset === "250" ? "true" : "false");
};
const syncGridCenterModeButton = () => {
  if (!gizmoGridCenterModeBtnEl) return;
  const mode = (viewer.getGridCenterMode?.() ?? "origin") as "origin" | "model";
  const isModel = mode === "model";
  gizmoGridCenterModeBtnEl.textContent = isModel ? "Center: Model" : "Center: Origin";
  gizmoGridCenterModeBtnEl.setAttribute("aria-pressed", isModel ? "true" : "false");
};
const syncGridColorPresetButtons = () => {
  gizmoGridColorPresetDefaultBtnEl?.setAttribute("aria-pressed", gridColorPresetState === "default" ? "true" : "false");
  gizmoGridColorPresetBlackBtnEl?.setAttribute("aria-pressed", gridColorPresetState === "black" ? "true" : "false");
...
const syncGridColorInputsFromViewer = () => {
  ...
  if (gizmoGridColorMajorYEl) gizmoGridColorMajorYEl.value = read("major", "y");
  if (gizmoGridColorMinorXEl) gizmoGridColorMinorXEl.value = read("minor", "x");
  if (gizmoGridColorMinorYEl) gizmoGridColorMinorYEl.value = read("minor", "y");
};
const captureGridColorSnapshotFromViewer = () => ({
  doubleMajor: {
    x: viewer.getGridLineColor?.("doubleMajor", "x") ?? "#000000",
    y: viewer.getGridLineColor?.("doubleMajor", "y") ?? "#000000",
  },
  major: {
    x: viewer.getGridLineColor?.("major", "x") ?? "#000000",
    y: viewer.getGridLineColor?.("major", "y") ?? "#000000",
  },
  minor: {
    x: viewer.getGridLineColor?.("minor", "x") ?? "#000000",
    y: viewer.getGridLineColor?.("minor", "y") ?? "#000000",
  },
});
const restoreGridColorSnapshot = (snapshot: {
  doubleMajor: { x: string; y: string };
  major: { x: string; y: string };
  minor: { x: string; y: string };
} | null) => {
  if (!snapshot) return;
  viewer.setGridLineColor?.("doubleMajor", "x", snapshot.doubleMajor.x);
  viewer.setGridLineColor?.("doubleMajor", "y", snapshot.doubleMajor.y);
  viewer.setGridLineColor?.("major", "x", snapshot.major.x);
  viewer.setGridLineColor?.("major", "y", snapshot.major.y);
  viewer.setGridLineColor?.("minor", "x", snapshot.minor.x);
  viewer.setGridLineColor?.("minor", "y", snapshot.minor.y);
  gridColorPresetState = viewer.getGridColorPreset?.() ?? "custom";
  syncGridColorInputsFromViewer();
  syncGridColorPresetButtons();
};
const applyGridColorPreset = (preset: "default" | "black" | "green_red") => {
  viewer.setGridColorPreset?.(preset);
  gridColorPresetState = viewer.getGridColorPreset?.() ?? preset;
  syncGridColorInputsFromViewer();
  syncGridColorPresetButtons();
};
const syncGridColorStateFromViewer = () => {
  gridColorPresetState = viewer.getGridColorPreset?.() ?? gridColorPresetState;
  syncGridColorInputsFromViewer();
  syncGridColorPresetButtons();
};
const applyBackgroundModeUi = (requested: "dark_blue" | "black" | "white") => {
  ...
  viewer.setBackgroundMode?.(requested);
  if (requested === "white") {
    gridColorPresetBeforeWhite = gridColorPresetState;
    gridColorSnapshotBeforeWhite = captureGridColorSnapshotFromViewer();
    applyGridColorPreset("black");
  } else if (prevMode === "white") {
    if (gridColorPresetBeforeWhite === "custom") {
      restoreGridColorSnapshot(gridColorSnapshotBeforeWhite);
    } else {
      applyGridColorPreset(gridColorPresetBeforeWhite);
    }
    gridColorSnapshotBeforeWhite = null;
  }
  syncViewBackgroundButtons();
};
const setScenesFeatureEnabled = (enabled: boolean) => {
  ...
  if (sceneFeatureEnabled) {
    ...
    viewer.setVisualSceneMode?.("off");
    applyBackgroundModeUi(backgroundBeforeScene);
  }
  syncViewBackgroundButtons();
};
...
if (gizmoGridToggleEl) {
  ...
  gizmoGridToggleEl.addEventListener("change", () => {
    const enabled = gizmoGridToggleEl.checked;
    viewer.setGridVisible?.(enabled);
    if (enabled) applyGridPreset("300");
    syncViewBackgroundButtons();
  });
}
if (gizmoGridCenterModeBtnEl) {
  gizmoGridCenterModeBtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    const mode = (viewer.getGridCenterMode?.() ?? "origin") as "origin" | "model";
    viewer.setGridCenterMode?.(mode === "model" ? "origin" : "model");
...
if (gizmoGridPresetInfiniteBtnEl) {
  gizmoGridPresetInfiniteBtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    applyGridPreset("infinite");
  });
}
if (gizmoGridPreset300BtnEl) {
  gizmoGridPreset300BtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    applyGridPreset("300");
  });
}
if (gizmoGridPreset250BtnEl) {
  gizmoGridPreset250BtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    applyGridPreset("250");
  });
...
if (gizmoGridMajorSpacingRangeEl) {
  const apply = () => {
    ...
    const major = Number.isFinite(v) ? Math.max(1, Math.min(200, v)) : 10;
    const minor = Number(gizmoGridMinorSpacingRangeEl?.value ?? viewer.getGridMinorSpacingMm?.() ?? 1);
    const clamped = Math.max(major, Number.isFinite(minor) ? minor : 1);
    if (gizmoGridMajorSpacingRangeEl.value !== String(clamped)) {
      gizmoGridMajorSpacingRangeEl.value = String(clamped);
    }
    viewer.setGridMajorSpacingMm?.(clamped);
    if (gizmoGridDoubleMajorSpacingRangeEl) {
      const dbl = Number(gizmoGridDoubleMajorSpacingRangeEl.value);
      if (Number.isFinite(dbl) && dbl < clamped) {
        gizmoGridDoubleMajorSpacingRangeEl.value = String(clamped);
      }
  ...
  gizmoGridMajorSpacingRangeEl.value = String(Math.min(200, Math.max(1, current)));
  apply();
}
if (gizmoGridMinorSpacingRangeEl) {
  const apply = () => {
    const v = Number(gizmoGridMinorSpacingRangeEl.value);
    const minor = Number.isFinite(v) ? Math.max(1, Math.min(100, v)) : 1;
    viewer.setGridMinorSpacingMm?.(minor);
    if (gizmoGridMajorSpacingRangeEl) {
      const major = Number(gizmoGridMajorSpacingRangeEl.value);
      if (Number.isFinite(major) && major < minor) {
        gizmoGridMajorSpacingRangeEl.value = String(minor);
      }
      viewer.setGridMajorSpacingMm?.(Number(gizmoGridMajorSpacingRangeEl.value));
    }
    if (gizmoGridDoubleMajorSpacingRangeEl) {
      const majorNow = Number(gizmoGridMajorSpacingRangeEl?.value ?? minor);
      const dbl = Number(gizmoGridDoubleMajorSpacingRangeEl.value);
      if (Number.isFinite(dbl) && Number.isFinite(majorNow) && dbl < majorNow) {
        gizmoGridDoubleMajorSpacingRangeEl.value = String(majorNow);
      }
      viewer.setGridDoubleMajorSpacingMm?.(Number(gizmoGridDoubleMajorSpacingRangeEl.value));
    }
  };
  ...
  gizmoGridMinorSpacingRangeEl.value = String(Math.min(100, Math.max(1, current)));
  apply();
}
const syncDoubleMajorPresetButtons = () => {
  if (!gizmoGridDoubleMajorPresetRowEl || !gizmoGridDoubleMajorSpacingRangeEl) return;
  const current = Math.round(Number(gizmoGridDoubleMajorSpacingRangeEl.value) || 0);
  const buttons = gizmoGridDoubleMajorPresetRowEl.querySelectorAll<HTMLButtonElement>("button[data-value]");
  buttons.forEach((btn) => {
    const target = Math.round(Number(btn.dataset.value) || 0);
    btn.setAttribute("aria-pressed", target === current ? "true" : "false");
  });
};
if (gizmoGridDoubleMajorSpacingRangeEl) {
  const apply = () => {
    const v = Number(gizmoGridDoubleMajorSpacingRangeEl.value);
    const major = Number(gizmoGridMajorSpacingRangeEl?.value ?? viewer.getGridMajorSpacingMm?.() ?? 10);
    const candidate = Number.isFinite(v) ? Math.max(1, Math.min(400, v)) : 20;
    const clamped = Math.max(candidate, Number.isFinite(major) ? major : 10);
    if (gizmoGridDoubleMajorSpacingRangeEl.value !== String(clamped)) {
      gizmoGridDoubleMajorSpacingRangeEl.value = String(clamped);
    }
    viewer.setGridDoubleMajorSpacingMm?.(clamped);
    syncDoubleMajorPresetButtons();
  };
  gizmoGridDoubleMajorSpacingRangeEl.addEventListener("input", apply);
  const current = Math.round(viewer.getGridDoubleMajorSpacingMm?.() ?? 100);
  gizmoGridDoubleMajorSpacingRangeEl.value = String(Math.min(400, Math.max(1, current)));
  apply();
}
if (gizmoGridDoubleMajorPresetRowEl && gizmoGridDoubleMajorSpacingRangeEl) {
  gizmoGridDoubleMajorPresetRowEl.addEventListener("click", (e) => {
    const btn = (e.target as HTMLElement | null)?.closest?.("button[data-value]") as HTMLButtonElement | null;
    if (!btn) return;
    e.preventDefault();
    e.stopPropagation();
    const raw = Number(btn.dataset.value);
    if (!Number.isFinite(raw)) return;
    const min = Number(gizmoGridDoubleMajorSpacingRangeEl.min);
    const max = Number(gizmoGridDoubleMajorSpacingRangeEl.max);
    const minV = Number.isFinite(min) ? min : 1;
    const maxV = Number.isFinite(max) ? max : 400;
    const next = Math.round(Math.max(minV, Math.min(maxV, raw)));
    gizmoGridDoubleMajorSpacingRangeEl.value = String(next);
    gizmoGridDoubleMajorSpacingRangeEl.dispatchEvent(new Event("input", { bubbles: true }));
  });
}
const bindGridSpacingQuickRow = (rowEl: HTMLDivElement | null, rangeEl: HTMLInputElement | null) => {
  if (!rowEl || !rangeEl) return;
  rowEl.addEventListener("click", (e) => {
    const btn = (e.target as HTMLElement | null)?.closest?.("button[data-mode][data-value]") as HTMLButtonElement | null;
    if (!btn) return;
    e.preventDefault();
    e.stopPropagation();
    ...
    const nextRaw = mode === "mul" ? curV * raw : raw;
    const next = Math.round(Math.max(minV, Math.min(maxV, nextRaw)));
    rangeEl.value = String(next);
    rangeEl.dispatchEvent(new Event("input", { bubbles: true }));
  });
};
bindGridSpacingQuickRow(gizmoGridMajorQuickRowEl, gizmoGridMajorSpacingRangeEl);
bindGridSpacingQuickRow(gizmoGridMinorQuickRowEl, gizmoGridMinorSpacingRangeEl);
if (gizmoGridMajorOpacityRangeEl) {
  const apply = () => {
    const v = Number(gizmoGridMajorOpacityRangeEl.value);
    viewer.setGridMajorOpacity?.(Number.isFinite(v) ? Math.max(0, Math.min(1, v / 100)) : 0.58);
  };
  gizmoGridMajorOpacityRangeEl.addEventListener("input", apply);
  const current = Math.round((viewer.getGridMajorOpacity?.() ?? 0.58) * 100);
  gizmoGridMajorOpacityRangeEl.value = String(Math.min(100, Math.max(0, current)));
  apply();
}
if (gizmoGridMinorOpacityRangeEl) {
  const apply = () => {
    const v = Number(gizmoGridMinorOpacityRangeEl.value);
    viewer.setGridMinorOpacity?.(Number.isFinite(v) ? Math.max(0, Math.min(1, v / 100)) : 0.45);
  };
  gizmoGridMinorOpacityRangeEl.addEventListener("input", apply);
  const current = Math.round((viewer.getGridMinorOpacity?.() ?? 0.45) * 100);
  gizmoGridMinorOpacityRangeEl.value = String(Math.min(100, Math.max(0, current)));
  apply();
}
if (gizmoGridDoubleMajorOpacityRangeEl) {
  const apply = () => {
    const v = Number(gizmoGridDoubleMajorOpacityRangeEl.value);
    viewer.setGridDoubleMajorOpacity?.(Number.isFinite(v) ? Math.max(0, Math.min(1, v / 100)) : 0.82);
  };
  gizmoGridDoubleMajorOpacityRangeEl.addEventListener("input", apply);
  const current = Math.round((viewer.getGridDoubleMajorOpacity?.() ?? 0.82) * 100);
  gizmoGridDoubleMajorOpacityRangeEl.value = String(Math.min(100, Math.max(0, current)));
  apply();
}
const bindGridColorInput = (
  input: HTMLInputElement | null,
  tier: "doubleMajor" | "major" | "minor",
  axis: "x" | "y"
) => {
  if (!input) return;
  input.addEventListener("input", () => {
    viewer.setGridLineColor?.(tier, axis, input.value);
    gridColorPresetState = viewer.getGridColorPreset?.() ?? "custom";
    syncGridColorPresetButtons();
  });
};
bindGridColorInput(gizmoGridColorDoubleMajorXEl, "doubleMajor", "x");
bindGridColorInput(gizmoGridColorDoubleMajorYEl, "doubleMajor", "y");
bindGridColorInput(gizmoGridColorMajorXEl, "major", "x");
bindGridColorInput(gizmoGridColorMajorYEl, "major", "y");
bindGridColorInput(gizmoGridColorMinorXEl, "minor", "x");
bindGridColorInput(gizmoGridColorMinorYEl, "minor", "y");
if (gizmoGridColorPresetDefaultBtnEl) {
  gizmoGridColorPresetDefaultBtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    applyGridColorPreset("default");
  });
}
if (gizmoGridColorPresetBlackBtnEl) {
  gizmoGridColorPresetBlackBtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    applyGridColorPreset("black");
  });
}
if (gizmoGridColorPresetGreenRedBtnEl) {
  gizmoGridColorPresetGreenRedBtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    applyGridColorPreset("green_red");
  });
}
...
if (gizmoViewLeftBtnEl) {
  gizmoViewLeftBtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    viewer.snapToLeftView?.();
  });
}
if (gizmoViewRightBtnEl) {
  gizmoViewRightBtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    viewer.snapToRightView?.();
  });
}
if (gizmoViewRearBtnEl) {
  gizmoViewRearBtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    viewer.snapToRearView?.();
  });
}
const syncVisualSceneUI = () => {
  const mode = (viewer.getVisualSceneMode?.() ?? "off") as "off" | "stars" | "nebula" | "swarm";
  if (sceneFeatureEnabled && mode === "off") {
    viewer.setVisualSceneMode?.(sceneModeWhenEnabled);
  }
  const wasActive = lastVisualSceneMode !== "off";
  const isActive = sceneFeatureEnabled && mode !== "off";
  ...
  gizmoStarsControlsEl?.classList.toggle("open", sceneFeatureEnabled && mode === "stars");
  gizmoNebulaControlsEl?.classList.toggle("open", sceneFeatureEnabled && mode === "nebula");
...
if (gizmoViewStarsBtnEl) {
  gizmoViewStarsBtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    sceneModeWhenEnabled = "stars";
    if (!sceneFeatureEnabled) setScenesFeatureEnabled(true);
    else viewer.setVisualSceneMode?.("stars");
    syncVisualSceneUI();
  });
}
if (gizmoViewNebulaBtnEl) {
  gizmoViewNebulaBtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    sceneModeWhenEnabled = "nebula";
    if (!sceneFeatureEnabled) setScenesFeatureEnabled(true);
    else viewer.setVisualSceneMode?.("nebula");
    syncVisualSceneUI();
  });
}
if (gizmoViewSwarmBtnEl) {
  gizmoViewSwarmBtnEl.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    sceneModeWhenEnabled = "swarm";
    if (!sceneFeatureEnabled) setScenesFeatureEnabled(true);
    else viewer.setVisualSceneMode?.("swarm");
    syncVisualSceneUI();
  });
}
syncVisualSceneUI();
...
function sampleProfilePatchWorldAtStation(
  st: { pt: Pt; tan: Pt },
  endX: number,
  endZ: number,
  p1s: number,
  ...
  const out: XYZ[] = [];
  const n = clampInt(samples, 8, 120);
  for (let i = 0; i <= n; i++) {
    const t = i / n;
    const p = bezier3_2d(P0, P1, P2, P3, t);
    const w = localToWorldAtStation(st, p);
    out.push({ x: w.x, y: w.y, z: w.z });
  }
  const w0 = localToWorldAtStation(st, P0);
  out.push({ x: w0.x, y: w0.y, z: w0.z });
  return out;
}

function isFiniteXYZ(p: XYZ | undefined | null): p is XYZ {
...
function updateArcViz() {
  ...
  try {
    ...
    if (frameOk && handlesOk && patchOk) {
      ...
      viewer.setProfileAEditHandleMetadata([
        { id: "P1", world: A_handlePtsW[0] },
...
function setInputValueWithEvents(el: HTMLInputElement, value: string, emitChange: boolean) {
  el.value = value;
  const pairedVal = document.getElementById(`${el.id}Val`);
  if (pairedVal instanceof HTMLInputElement) pairedVal.value = value;
  el.dispatchEvent(new Event("input", { bubbles: true }));
  if (emitChange) el.dispatchEvent(new Event("change", { bubbles: true }));
}

function normalizeSignedDeg180(deg: number) {
  let out = Number.isFinite(deg) ? deg : 0;
  while (out > 180) out -= 360;
  while (out < -180) out += 360;
  return out;
}

function commitProfileAHandleDragToInputs(allLocal: Record<"P1" | "P2" | "P3", { u: number; z: number }>) {
  const p1 = allLocal.P1;
  const p2 = allLocal.P2;
  const p3 = allLocal.P3;
  if (!p1 || !p2 || !p3) return;
  const p1s = clamp(p1.z, 0, 10000);
  const endX = clamp(Math.abs(p3.u), 0.1, 2000);
  const endZ = clamp(p3.z, 0.1, 2000);
  const span = Math.hypot(p3.u, p3.z);
  const dx = p3.u - p2.u;
  const dz = p3.z - p2.z;
  const p3Distance = Math.hypot(dx, dz);
  const p3s = clamp(p3Distance, 0, Math.max(1, span * 1.5));
  const fallbackEnda = clamp(readNumber(mustEl<HTMLInputElement>("toe_a_enda"), 0), -180, 180);
  const solvedEnda = p3Distance < 1e-6 ? fallbackEnda : normalizeSignedDeg180((Math.atan2(dz, dx) * 180) / Math.PI + 180);

  const assignments: Array<[id: string, value: string]> = [
    ["toe_a_p1s", p1s.toFixed(3)],
    ["toe_a_endx", endX.toFixed(3)],
    ["toe_a_endz", endZ.toFixed(3)],
    ["toe_a_p3s", p3s.toFixed(3)],
    ["toe_a_enda", clamp(solvedEnda, -180, 180).toFixed(3)],
  ];

  suppressParamChangeRebuild = true;
  try {
    for (let i = 0; i < assignments.length; i++) {
      const [id, value] = assignments[i];
      const el = document.getElementById(id);
      if (!(el instanceof HTMLInputElement)) continue;
      setInputValueWithEvents(el, value, true);
    }
  } finally {
    suppressParamChangeRebuild = false;
  }
...
onLayoutModeChanged = (isMobile) => {
  if (isMobile) {
    ...
    setToolbarState(getLayoutDefaultCollapsedState(true), null);
    return;
  }
  captureCurrentGizmoSizeForState();
  setToolbarState(getLayoutDefaultCollapsedState(false), null);
...
([tolEl, ...paramEls]).forEach((el) => {
  ...
  el.addEventListener("change", () => {
    ...
    updateArcViz();
    applySectionCutUIToViewer();
    if (suppressParamChangeRebuild) return;
    if (!canAutoRebuild()) return;
    markParamChangeRebuildSource();
    if (rebuildAsOneEl.checked) {
      crossRefitPendingManual = false;
