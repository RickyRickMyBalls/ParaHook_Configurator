...
describe('selectSharedPreviewRenderVm', () => {
  ...
  it('does not revive the coarse parent artifact for unresolved explicit contributors during shared composition', () => {
    const explicitPreparation: GraphPreviewPreparation = {
      outputPreviewNodeId: 'node-output-preview-1',
      outputSlotIds: ['s001'],
      previewCandidateSlotIds: ['s001'],
      previewCandidatePartKeys: ['extrude'],
    ...
    const sharedVm = selectSharedPreviewRenderVm([
      {
        graphDocumentId: 'graph-document-1',
        previewPreparation: explicitPreparation,
        buildOutputs: [coarseArtifact],
        buildBundle: acceptedBundle,
      },
    ])
    ...
    expect(sharedVm.items.map((item) => ({
      outputEntryId: item.outputEntryId,
      viewerPart: item.viewerPart?.artifact.partKeyStr ?? null,
    }))).toEqual([
      {
        outputEntryId: 'output-entry:s001:node-extrude-1:port-SolidBody%3A001',
        viewerPart: 'extrude:node-extrude-1:body:001',
      },
      {
